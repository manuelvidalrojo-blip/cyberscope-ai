...
<div class="info-box">
    ...
    configuración HTTPS, DNS, SPF, DMARC y políticas de seguridad.
...
if analyze:
    ...
    if not domain.strip():
        ...
        with st.spinner(f"Analizando {clean_domain}…"):
            ...
            dns_result = analyze_dns(clean_domain)
        ...
        <div class="score-card">
            ...
            <div class="score-number" style="color:{score_color};">
                {score}<span style="font-size:1.5rem;color:#3A4A5C;">/100</span>
            </div>
            <br>
        ...
        col1.metric("HTTP Status", str(http_status) if http_status else "N/A")
        col2.metric("HTTPS Status", str(https_status) if https_status else "N/A")
        col3.metric("HTTPS activo", "✅ Sí" if uses_https else "❌ No")
        ...
        # ── 4. DNS y correo ───────────────────────────────────────────────────
        st.markdown('<p class="section-title">◈ DNS y correo</p>', unsafe_allow_html=True)

        dns_col1, dns_col2 = st.columns(2)
        dns_col1.metric("SPF detectado", "✅ Sí" if dns_result["spf_found"] else "❌ No")
        dns_col2.metric("DMARC detectado", "✅ Sí" if dns_result["dmarc_found"] else "❌ No")

        with st.expander("🌐 Ver registros DNS"):
            st.write("**Registros A:**", dns_result["a_records"])
            st.write("**Registros AAAA:**", dns_result["aaaa_records"])
            st.write("**Registros MX:**", dns_result["mx_records"])
            st.write("**Registros NS:**", dns_result["ns_records"])
            st.write("**Registros TXT:**", dns_result["txt_records"])
            st.write("**DMARC:**", dns_result["dmarc_record"])
        ...
        if not dns_result["spf_found"]:
            st.warning("No se ha detectado SPF. Se recomienda publicar un registro SPF para proteger el correo frente a suplantación.")

        if not dns_result["dmarc_found"]:
            st.warning("No se ha detectado DMARC. Se recomienda publicar una política DMARC para mejorar la protección del dominio.")

        if not scan_result.get("redirects_to_https") and not scan_result.get("error"):
            st.info("ℹ️ El dominio no redirige automáticamente de HTTP a HTTPS. Considera añadir una redirección 301 en tu servidor.")

        # ── 6. Descargar informe HTML ─────────────────────────────────────────
        ...
        with st.expander("🔍 Ver datos técnicos JSON"):
            ...
            st.caption("Resultado del análisis DNS")
            st.json(dns_result)
